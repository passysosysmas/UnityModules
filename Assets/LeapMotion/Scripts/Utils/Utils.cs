...
namespace Leap.Unity {
  ...
  public static class Utils {
    ...
    public static void DrawCircle(RuntimeGizmoDrawer drawer,
                                  Vector3 center,
                                  Vector3 normal,
                                  float radius,
                                  Color color,
      ...
      DrawArc(drawer, 360, center, planeA, normal, radius, color, quality);
    }
    ...
    public static void DrawArc(RuntimeGizmoDrawer drawer,
                               float arc,
                               Vector3 center,
                               Vector3 forward,
                               Vector3 normal,
      ...
      for (float angle = 0; Mathf.Abs(angle) <= Mathf.Abs(arc); angle += deltaAngle) {
        ...
        drawer.color = color;
        drawer.DrawLine(thisPoint, nextPoint);
        thisPoint = nextPoint;
      }
    }

    public static void DrawCone(RuntimeGizmoDrawer drawer, 
                                Vector3 origin,
                                Vector3 direction,
                                float angle,
                                float height,
      ...
      for (float q = step; q <= height; q += step) {
        DrawCircle(drawer, origin + direction * q, direction, Mathf.Tan(angle * Constants.DEG_TO_RAD) * q, color, quality * 8);
      }
    }
